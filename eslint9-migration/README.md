...
**Goal:**
Automate the migration of the specified monorepo from its current ESLint 8 setup to an ESLint 9 setup using `eslint.config.ts` flat configurations.
...
1.  **Strictly Follow the Guide:** Adhere to all phases, steps, and "LLM Action" items detailed in the "LLM-Guided ESLint 8 to ESLint 9 Monorepo Migration" guide. The guide is tailored for creating `eslint.config.ts` files.
...
4.  **Incremental Progress & Reporting:**
    ...
    *   For each significant action taken (e.g., modifying a `package.json`, creating an `eslint.config.ts` file), briefly state the action and its outcome.
...
8.  **Final Deliverables:**
    ...
    *   All legacy ESLint configurations (`.eslintrc.*`, `package.json#eslintConfig`) replaced with `eslint.config.ts` files.
...
*   Acknowledge that the migration will produce `eslint.config.ts` files as per the guide.
